...
| [null_resource.wait_for_attachments_cleared](https://registry.terraform.io/providers/hashicorp/null/latest/docs/resources/resource) | resource |
| [null_resource.wait_for_tgw_available](https://registry.terraform.io/providers/hashicorp/null/latest/docs/resources/resource) | resource |
...
| <a name="output_tgw_id"></a> [tgw\_id](#output\_tgw\_id) | depends\_on here (not just the value expression) is what makes this actually work: everything that reads tgw\_id — the egress VPC's own attachment, the SSM parameter spokes read, all of it — waits for null\_resource.wait\_for\_tgw\_available to finish first, without each of those callers needing to know the wait exists. |
| <a name="output_tgw_route_table_ids"></a> [tgw\_route\_table\_ids](#output\_tgw\_route\_table\_ids) | Same reasoning as tgw\_id above — these feed the route table associations/propagations that attachments need, so they wait too. |
<!-- END_TF_DOCS -->
