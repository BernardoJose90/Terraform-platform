...
| Name | Description |
| ---- | ----------- |
| <a name="output_route_table_ids"></a> [route\_table\_ids](#output\_route\_table\_ids) | Route table ID keyed by workload, e.g. route\_table\_ids["rds"] — one per top-level key in var.production\_workload\_subnets. |
| <a name="output_subnet_ids"></a> [subnet\_ids](#output\_subnet\_ids) | Subnet ID keyed by "<workload>-<az\_key>", e.g. subnet\_ids["eks-a"]. Combines each workload key in var.production\_workload\_subnets with its per-AZ subnets map key. |
<!-- END_TF_DOCS -->
